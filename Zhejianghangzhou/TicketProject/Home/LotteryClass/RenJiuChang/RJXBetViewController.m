//
//  RJXBetViewController.m
//  TicketProject
//
//  Created by 刘坤 on 16/3/2.
...
@interface RJXBetViewController ()
@property(nonatomic,copy)NSString *allstr;
@end

@implementation RJXBetViewController
...
- (UIViewController *)pushViewCOntrollerWithSelectedBallsDic:(NSDictionary *)ballsDic lotteryDic:(NSDictionary *)dic {
    RJXViewController *detailView = [[RJXViewController alloc] initWithSelectedBallsDic:ballsDic LotteryDic:dic];
...
- (void)clearSelect:(id)sender {
    ...
    NSString *formateSelectBalls = [RJCParserNumber getRJCTextWithDict:_selectMatchDic matchCount:kMatchCount];
    
    NSInteger betCount = [RJCParserNumber getCountForRJXWithSelectMatchDic:_selectMatchDic];
...
- (void)updateBottomView {
    if ([self queryMatchCount] < 9) {
        ...
        _betCount = [RJCParserNumber getCountForRJXWithSelectMatchDic:_selectMatchDic];              // 总注数
    }
